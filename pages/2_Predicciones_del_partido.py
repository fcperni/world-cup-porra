...
with safe_page():
    ...
    from ui_common import PHASE_LABELS, configure_page, fmt, get_data, get_results, proper_name
    ...
    def label(num: int) -> str:
        m = by_num[num]
        h, a = m_names(m)
        return f"{m_tag(m)} · {h} – {a} · {short_day(m)}"

    # Partido por defecto: el que viene en la URL (al pinchar en el calendario); si no,
    # el próximo sin resultado (el que se está jugando o el siguiente), o el último.
    ...
    pen_html = ""
    if played and m.phase.is_knockout and g[0] == g[1] and m.number in results.ko_winners:
        side = results.ko_winners[m.number]
        winner = home if side == "home" else away
        pen_html = f'<div class="prd-pen">Pasa en los penaltis: {flag_img(winner, 12)} {winner}</div>'
